...
namespace TodoList_App
{
    public class Model
    {
        ...
        public bool EditTask(string newName, string previousName)
        {
            ...
            string query = "UPDATE `t_task` SET `name` = @newName WHERE `name` = @previousName && `user_id` = @user_id;";
            cmd = new MySqlCommand(query, Connection);
        ...
        public bool EraseTask(string name)
        {
            ...
            string query = "DELETE FROM `t_task` WHERE `name` = @name AND `user_id` = @user_id;";
            cmd = new MySqlCommand(query, Connection);
            cmd.Parameters.AddWithValue("@name", name);
            cmd.Parameters.AddWithValue("@user_id", userID);
            cmd.Prepare();
